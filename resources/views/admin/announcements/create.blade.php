<x-app-layout>
    ...
    <div class="max-w-3xl mx-auto">
        <form action="{{ route('admin.announcements.store') }}" method="POST" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-6">
            @csrf
            ...
            <div>
                ...
                <textarea name="content" rows="5" required class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('content') }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="target_audience" required class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        <option value="admin" {{ old('target_audience') == 'admin' ? 'selected' : '' }}>Hanya Admin</option>
                        <option value="guru" {{ old('target_audience') == 'guru' ? 'selected' : '' }}>Hanya Guru</option>
                        <option value="siswa" {{ old('target_audience') == 'siswa' ? 'selected' : '' }}>Hanya Siswa</option>
                    </select>
                </div>
                <div>
                    ...
                    <select name="urgency_level" required class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        <option value="important" {{ old('urgency_level') == 'important' ? 'selected' : '' }}>Penting (Important)</option>
                        <option value="urgent" {{ old('urgency_level') == 'urgent' ? 'selected' : '' }}>Mendesak (Urgent)</option>
                    </select>
                </div>
            </div>
            ...
            <div class="pt-4 flex justify-end gap-3">
                ...
                <button type="submit" class="px-5 py-2.5 bg-indigo-600 text-white rounded-xl font-medium hover:bg-indigo-700 transition-colors">Simpan Pengumuman</button>
            </div>
        </form>
    </div>
</x-app-layout>
