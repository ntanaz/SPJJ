                ...
                <div class="bg-gradient-to-br from-indigo-50 to-blue-50 rounded-3xl p-6 shadow-sm border border-indigo-100">
                    ...
                    @if($isPastDeadline && !$submission)
                        <div class="text-center py-6">
                            <div class="inline-flex h-12 w-12 items-center justify-center rounded-full bg-red-100 text-red-500 mb-3">
                            ...
                            <p class="font-bold text-red-600">Terlambat!</p>
                            <p class="text-sm text-red-500 mt-1">Batas waktu telah berakhir, tidak dapat menerima kiriman baru.</p>
                        </div>
                    @else
                        @if($submission && $isPastDeadline)
                            <!-- Already submitted but past deadline (can't resubmit) -->
                            <div class="text-center py-4 bg-white/60 rounded-xl border border-white">
                                <p class="text-emerald-600 font-bold mb-1">✅ Berkas berhasil terekam sistem</p>
                                <a href="{{ asset('storage/' . $submission->file_path) }}" target="_blank" class="text-sm text-indigo-600 hover:underline font-medium block mb-2">Lihat File Terkirim</a>
                                <p class="text-xs text-gray-500 italic">Pengumpulan ditutup karena batas waktu berakhir.</p>
                            </div>
                        @else
                            ...
                            <form action="{{ route('student.assignments.submit', $assignment) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                                ...
                                @if($submission)
                                    <div class="text-center py-2 bg-indigo-100/50 rounded-xl mb-4 border border-indigo-200">
                                        <p class="text-xs text-indigo-600 font-bold">Anda sudah mengumpulkan berkas.</p>
                                        <a href="{{ asset('storage/' . $submission->file_path) }}" target="_blank" class="text-sm font-bold text-indigo-800 hover:underline mt-1 block">Lihat File Anda</a>
                                        <p class="text-xs text-indigo-500 mt-1">Mengupload kembali akan menimpa file sebelumnya.</p>
                                    </div>
                                @endif
                                ...
                                <div>
                                    ...
                                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-indigo-200 border-dashed rounded-2xl bg-white hover:bg-gray-50 transition-colors">
                                        <div class="space-y-1 text-center">
                                            ...
                                            <p class="text-xs text-gray-500">PDF, DOCX, ZIP s/d 10MB</p>
                                        </div>
                                    </div>
                                    @error('file')<p class="mt-2 text-sm text-red-600 font-bold">{{ $message }}</p>@enderror
                                </div>
